...
class Weather:
    ...
    def update(self):
        ...
        self.screen.graph("T:", self.temps, self.screen.lcd.width//2-10, 43, 30, top=False)
        self.screen.text_center("%.0f" % min(self.temps), 145, 38, BLACK)
        self.screen.text_center("%.0f" % max(self.temps), 145, 48, BLACK)

        self.screen.graph("H:", self.humids, self.screen.lcd.width//2-10, 77, 30, top=False)
        self.screen.text_center("%.0f" % min(self.humids), 145, 73, BLACK)
        self.screen.text_center("%.0f" % max(self.humids), 145, 82, BLACK)
        
        self.screen.graph("B:", self.baroms, self.screen.lcd.width//2-10, 111, 30, top=False)
